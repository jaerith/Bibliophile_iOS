//
//  BooksByCurrentLocationController.m
//  BiblioPhile
//
...
@implementation BooksByCurrentLocationController
...
- (void) retrieveData {
	...
	// Fill out the list
	NSString *targetFile;
	float fLatitude  = [delegate.currLatitude floatValue];
	float fLongitude = [delegate.currLongitude floatValue];
		
	if ((fLatitude >= 38.8397f && fLatitude <= 59.3555f) && 
	...
	for (id titleKey in titleKeys) {
		...
		CLLocationDegrees ldCurrLatitude = 
			(CLLocationDegrees) [delegate.currLatitude doubleValue];
		
		CLLocationDegrees ldCurrLongitude =
			(CLLocationDegrees) [delegate.currLongitude doubleValue];
...
- (UITableViewCell*) tableView:(UITableView*)tableView
	...
	static NSString* ByCurrentLocationBooksCellIdentifier = @"ByCurrentLocationBooksCellIdentifier";
	
	UITableViewCell *cell = 
	    [tableView dequeueReusableCellWithIdentifier:ByCurrentLocationBooksCellIdentifier];
	
	if (cell == nil){
		cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero
									   reuseIdentifier:ByCurrentLocationBooksCellIdentifier] autorelease];
	}
	else {
